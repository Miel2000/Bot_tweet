...
# commande pour envoyer un tweet



def say_hello(client)

    client.update("Je suis un robot-lama venu des années -700.000 avant jc \n #bonjour_monde \n 🐐")

end

def sendTweet(client, pelos)

# ici le client.update(' bla bla bla @the_hacking_pro') a la place de puts 

client.update("🔹 Félicitation #{pelos.join(", ")} 🔹 \n Tu as été selectioné 👈\n Dans une liste de journaliste\n Talentueux et Indépendant👏\n Pour m'exercer\n A ma nouvelle vie de robot 🏊\n  @the_hacking_pro 🤖 ")
puts "°" * 50
puts " TWEET SENDED "
puts "°" * 50 
   ...
   # fonction qui choisi un nom dans la liste grace à .sample, initie un tableau, le remplie dans l'itération et le return dans randomHandle
...
##### POUR LIKE  25 TWEET AVEC UN MOT PRECIS client.favorite ######


def likeTweet(client)
...
##### POUR FOLLOW  25 TWEETER AVEC UN MOT PRECIS client.follow ######

def followTweet(client)
...
# commande pour afficher mes infos



def liking_and_follow_25_recents_bonjour(client)

  
    
    client.search('#bonjour_monde',reslut_type: 'recent').take(25).collect do |k|
      client.favorite(k.id)
      puts "VOUS AVEZ LIKEZ LE POST SUIVANT : \n°°°\n  #{k.text} \n°°°\n NUMERO : #{k.id}\n°°°\n AUTEUR : #{k.user.name.upcase}."
      client.follow(k.user.id)
      puts "vous suivez maintenant #{k.user.name}."
      sleep (30)
    end
  end
...
def result(client)
 ...
 puts "Localisation  de mon compte"
...
def perform
   ...
   liking_and_follow_25_recents_bonjour(login_twitter)
   # sendTweet(login_twitter, randomHandle)
   # likeTweet(login_twitter)
   # login_twitter
   # result(login_twitter)
   # say_hello(login_twitter)
   # followTweet(login_twitter)
